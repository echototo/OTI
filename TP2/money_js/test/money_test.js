...
QUnit.test("test construct money with false value",function(assert)
{
	assert.throws(function(assert) {new money(-1,"EUR")}, ValueLowerZeroExec, "Value can't be lower than 0");
}
);

QUnit.test("test construct money with false currency",function(assert)
{
	assert.throws(function(assert) {new money(1,"EURRR")}, ErrorCurrencyLength, "Currency too long");
}
);
